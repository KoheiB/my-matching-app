<template>
  <v-container>
    ...
    <v-tabs-items v-model="tab">
      ...
      <v-tab-item>
        <v-row>
          <v-col cols="6" sm="4" lg="3" v-for="getLike in getLikes" :key="getLike.id">
            <v-card
              hover
              nuxt
            ...
            >
              ...
              <v-card-actions>
                <v-btn @click.prevent="approveLike(getLike)"
                color="info"
                block
                rounded
                height="50"
      ...
      <v-tab-item>
        <v-row>
          <v-col cols="6" sm="4" lg="3" v-for="myLike in myLikes" :key="myLike.id">
            <v-card
              hover
              nuxt
              :to="`/users/${myLike.userId}`"
            >
              <v-card-title>
                {{ myLike.displayname }}
              </v-card-title>
    ...
    this.$fireAuth.onAuthStateChanged(async(user) => {
      ...
      try {
        ...
        getLikes.forEach(async (getLike) => {
          ...
          getLike.createdAt = getLike.createdAt.toDate().toLocaleString('ja-JP-u-ca-japanese')
      ...
      // 自分のいいねを取得。
      try {
        const querySnapshot = await this.$firestore.collection('users').doc(user.uid).collection('likedProfiles').get()
        ...
        myLikes.forEach(async (myLike) => {
          ...
          myLike.createdAt = myLike.createdAt.toDate().toLocaleString('ja-JP-u-ca-japanese')
